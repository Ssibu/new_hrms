...
// Accurate working day calculation to match the backend.
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">My Leave Allowance ({new Date().getFullYear()})</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {policies.length > 0 ? policies.map((policy) => {
            if (policy.category === 'Unpaid') {
              return (
                <div key={policy._id} className="bg-gray-50 p-4 rounded-lg">
                  <div className="flex justify-between items-center mb-2"><span className="font-medium text-gray-900">{getLeaveTypeName(policy.type)}</span><span className="text-sm px-2 py-1 rounded-full bg-yellow-100 text-yellow-800">{policy.category}</span></div>
                  <div className="flex justify-between items-center mt-2 pt-2"><span className="text-lg font-semibold text-gray-700">Available to Apply</span></div>
                </div>
              );
            }
            if (policy.renewalType === 'Yearly') {
              ...
              return (
                <div key={policy._id} className="bg-gray-50 p-4 rounded-lg">
                  <div className="flex justify-between items-center mb-2"><span className="font-medium text-gray-900">{getLeaveTypeName(balance.leaveType)}</span><span className="text-sm px-2 py-1 rounded-full bg-green-100 text-green-800">{policy.category}</span></div>
                  <div className="flex justify-between items-center"><span className="text-2xl font-bold text-blue-600">{available}</span><span className="text-sm text-gray-500">days available</span></div>
                  <div className="mt-2 text-sm text-gray-600">Used: {balance.used} / Total: {balance.total} (Yearly)</div>
                  <div className="mt-2 w-full bg-gray-200 rounded-full h-2"><div className="bg-blue-600 h-2 rounded-full" style={{ width: `${balance.total > 0 ? (balance.used / balance.total) * 100 : 0}%` }}></div></div>
                </div>
              );
            }
            if (policy.renewalType === 'Monthly') {
              return (
                <div key={policy._id} className="bg-gray-50 p-4 rounded-lg">
                  <div className="flex justify-between items-center mb-2"><span className="font-medium text-gray-900">{getLeaveTypeName(policy.type)}</span><span className="text-sm px-2 py-1 rounded-full bg-green-100 text-green-800">{policy.category}</span></div>
                  <div className="flex justify-between items-center"><span className="text-2xl font-bold text-blue-600">{policy.monthlyDayLimit}</span><span className="text-sm text-gray-500">days per month</span></div>
                  <div className="mt-2 text-sm text-gray-600">Granted each month (Monthly)</div>
                </div>
              );
            }
            return null;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select value={formData.leaveType} onChange={(e) => setFormData({ ...formData, leaveType: e.target.value })} className="w-full p-2 border border-gray-300 rounded-md" required>
                <option value="">Select Leave Type</option>
                {policies.map((policy) => {
                  if (policy.category === 'Unpaid') {
                    return <option key={policy.type} value={policy.type}>{getLeaveTypeName(policy.type)} (Unpaid)</option>;
                  }
                  if (policy.renewalType === 'Yearly') {
                    const balance = leaveBalance.find(b => b.leaveType === policy.type);
                    if (!balance || (balance.total - balance.used <= 0)) return null;
                    return <option key={policy.type} value={policy.type}>{getLeaveTypeName(policy.type)} (Available: {balance.total - balance.used})</option>;
                  }
                  if (policy.renewalType === 'Monthly') {
                    return <option key={policy.type} value={policy.type}>{getLeaveTypeName(policy.type)} ({policy.monthlyDayLimit} per month)</option>;
                  }
                  return null;
                })}
              </select>
            ...
            <div>
              ...
              <input type="text" value={calculateWorkingDays(formData.fromDate, formData.toDate)} className="w-full p-2 border border-gray-300 rounded-md bg-gray-50" readOnly/>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">From Date</label>
              <input type="date" value={formData.fromDate} onChange={(e) => setFormData({ ...formData, fromDate: e.target.value })} className="w-full p-2 border border-gray-300 rounded-md" min={new Date().toISOString().split('T')[0]} required/>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">To Date</label>
              <input type="date" value={formData.toDate} onChange={(e) => setFormData({ ...formData, toDate: e.target.value })} className="w-full p-2 border border-gray-300 rounded-md" min={formData.fromDate || new Date().toISOString().split('T')[0]} required/>
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Reason</label>
            <textarea value={formData.reason} onChange={(e) => setFormData({ ...formData, reason: e.target.value })} className="w-full p-2 border border-gray-300 rounded-md" rows="4" placeholder="Please provide a detailed reason for your leave request..." required/>
          </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {myRequests.length > 0 ? myRequests.map((request) => (
                <tr key={request._id}>
                  <td className="px-6 py-4 whitespace-nowrap"><span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">{getLeaveTypeName(request.leaveType)}</span></td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatDate(request.fromDate)} - {formatDate(request.toDate)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{request.numberOfDays} days</td>
                  <td className="px-6 py-4 whitespace-nowrap"><span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(request.status)}`}>{request.status}</span></td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(request.appliedAt)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button onClick={() => setSelectedRequest(request)} className="text-blue-600 hover:text-blue-900" title="View Details"><EyeIcon className="h-4 w-4" /></button>
                  </td>
                </tr>
              )) : (
                <tr>
                    <td colSpan="6" className="text-center py-10 text-gray-500">No leave requests found.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {selectedRequest && (
        <Modal open={!!selectedRequest} onClose={() => setSelectedRequest(null)}>
            ...
            <div className="flex justify-end mt-4"><button onClick={() => setSelectedRequest(null)} className="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600">Close</button></div>
        </Modal>
      )}
    </div>
  );
